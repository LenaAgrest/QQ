...
void PostOpen::InitializeComponent(void)
{
	...
	this->image_post->Size = System::Drawing::Size(500, 500);
	this->image_post->Dock = DockStyle::Fill;
...
{
	if (MessageBox::Show("Вы уверены, что хотите удалить этот пост?", "Подтверждение", MessageBoxButtons::YesNo, MessageBoxIcon::Question) == DialogResult::Yes)
	{
		bool success = PostRepository::DeletePost(this->PostData->ID);
		if (success) {
			if (OnPostDeleted != nullptr) {
				OnPostDeleted(); // вызвать обработчик
			}
		}
		else {
			MessageBox::Show("Ошибка при удалении поста.");
		}
	}
}
